...
async function canEditDepartment(departmentId: string) {
...
export default async function DepartmentPage({ params }: { params: { id: string } }) {
  ...
  try {
    // For now, we'll use a mapping of department IDs to their full names
    ...
    const canEdit = await canEditDepartment(departmentId)
        ...
        <div className="flex justify-between items-start mb-6">
          ...
          {canEdit && (
            <Button variant="outline" size="sm" asChild>
              <a href={`/admin/departments/${departmentId}/edit`} className="flex items-center">
                <svg
              ...
              <CardContent>
                <div className="flex justify-center mb-4">
                  <div className="relative w-40 h-40">
                    <Image
                      src={`/placeholder.svg?height=160&width=160&text=${departmentId}+Logo`}
                      alt={`${departmentId} Logo`}
                      width={160}
                      height={160}
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                      {[1, 2, 3, 4, 5, 6].map((i) => (
                        <div key={i} className="aspect-square relative">
                          <Image
                            src={`/placeholder.svg?height=200&width=200&text=${departmentId}+Image+${i}`}
                            alt={`${departmentId} Image ${i}`}
                            fill
